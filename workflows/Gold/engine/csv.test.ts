...
describe('emitCsv', () => {
  it('emits the exact 9-column header and CRLF rows', () => {
  ...
  it('a plain row needs no quoting and JSON-encodes the array cells', () => {
    ...
    const row = csv.split('\r\n')[1]
    expect(row).toBe(
      '1,,,,What is the diagnosis?,"[""Appendicitis"",""Cholecystitis""]",,[],no_answer_key',
    )
  })
  ...
  it('options containing quotes, commas, and newlines survive a full round-trip', () => {
    ...
    expect(JSON.parse(cells[5])).toEqual(nasty)
  ...
  it('image_urls emit as a JSON array cell with relative paths', () => {
    ...
    const cells = parseCsvLine(csv.split('\r\n')[1])
    expect(JSON.parse(cells[7])).toEqual([
      'images/asset01.jpg',
      'images/asset02.jpg',
    ])
  })
