...
function makeExamQuestion(id: string, overrides: Partial<ExamQuestion> = {}): ExamQuestion {
  return {
    id,
    topic: '',
    subtopic: '',
...
describe('validateFinalRows', () => {
  it('passes clean rows', () => {
    ...
    const rows = [makeExamQuestion('1'), makeExamQuestion('2')]
  ...
  it('fails when the row count does not match the blueprint', () => {
    const blueprint = makeBlueprint()
    const result = validateFinalRows([makeExamQuestion('1')], blueprint, NO_CROPS)
  ...
  it('fails an unflagged row with empty question text', () => {
    const blueprint = makeBlueprint()
    const rows = [
      makeExamQuestion('1', { question: '', needs_review: '' }),
      makeExamQuestion('2'),
    ]
  ...
  it('allows an empty question when the row is explicitly flagged', () => {
    const blueprint = makeBlueprint()
    const rows = [
      makeExamQuestion('1', { question: '', needs_review: 'mark_illegible' }),
      makeExamQuestion('2'),
    ]
  ...
  it('fails an unflagged MCQ row with no options', () => {
    const blueprint = makeBlueprint()
    const rows = [
      makeExamQuestion('1', { options: [], needs_review: '' }),
      makeExamQuestion('2'),
    ]
    const result = validateFinalRows(rows, blueprint, NO_CROPS)
    expect(result.ok).toBe(false)
  ...
  it('fails a correct_index that is out of range for its options', () => {
    const blueprint = makeBlueprint()
    const rows = [
      makeExamQuestion('1', { correct_index: '4', needs_review: '' }), // 4 options → 0..3
      makeExamQuestion('2'),
    ]
    const result = validateFinalRows(rows, blueprint, NO_CROPS)
    expect(result.ok).toBe(false)
  ...
  it('accepts index 0 and the last valid index', () => {
    const blueprint = makeBlueprint()
    const rows = [
      makeExamQuestion('1', { correct_index: '0', needs_review: '' }),
      makeExamQuestion('2', { correct_index: '3', needs_review: '' }),
    ]
    expect(validateFinalRows(rows, blueprint, NO_CROPS).ok).toBe(true)
  })
  ...
  it('fails a row whose image was never produced', () => {
    const blueprint = makeBlueprint()
    const rows = [
      makeExamQuestion('1', { image_urls: ['images/asset01.jpg'] }),
      makeExamQuestion('2'),
    ]
    const result = validateFinalRows(rows, blueprint, NO_CROPS)
    expect(result.ok).toBe(false)
  ...
  it('does not require options on a non-MCQ row (no options region planned)', () => {
    ...
    const rows = [makeExamQuestion('1', { options: [], needs_review: '' })]
    expect(validateFinalRows(rows, blueprint, NO_CROPS).ok).toBe(true)
  })
})
