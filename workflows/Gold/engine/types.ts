 ...
 * ENGINE.md §1 (semantics) and PROMPTS.md §2.1's blueprint JSON shape. Models
...
/**
 * The 9-column schema (§1.6, §3.1) — order is contract. It is both the list
 * the PLANNER must echo back in its blueprint and the column order of the
 * in-run `csv` artifact, and it is named verbatim in the SHA-pinned PLANNER
 * prompt, so it cannot change without editing that prompt and re-pinning.
 * Export does not read it: `export-csv.ts` re-projects from `merged-rows`
 * per the tutor's Customize columns.
 */
...
/**
 * One finished exam question — the unit every workflow must produce and
 * every downstream feature (Review, editing, topic matching, export, backup)
 * operates on. `correct_index` is a digit string or '' (§3.2).
 */
