...
describe('splitRow', () => {
  ...
  it('keeps the parent link and figures so split rows stay together', () => {
    const withFigure = makeRow({ image_urls: ['images/asset01.jpg'] })
    for (const row of splitRow(withFigure, split)) {
      // The `{parentId}~m{n}` id is what ties a split row to its parent —
      // it is how Review resolves the parent's source region.
      expect(parentRowId(row.id)).toBe(withFigure.id)
