...
describe('validateWorkerChunk', () => {
  ...
  it('rejects a changed planner-owned field', () => {
    const rows = chunkRowsOf(makeBlueprint())
    const body = JSON.parse(
      makeChunkResponse(rows, () => ({ topic: 'worker-invented-topic' })),
    )
    const result = validateWorkerChunk(JSON.stringify(body), rows)
    expect(result.ok).toBe(false)
    if (!result.ok) {
      expect(result.errors.join(' ')).toContain('planner-owned field "topic" was changed')
    }
  })
